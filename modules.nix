{
  ...
  cfg_zpool = config.zpool;
  types = lib.types;

  getArgs = module: let
    cleanCfg = lib.filterAttrs (name: value: value != null) (removeAttrs module ["output" "_module" "prefix"]);
...
in let
  zpoolprops = {config, ...}: {
    ...
    config = {
      output = let
        cleanCfg = lib.filterAttrs (name: value: value != null) (removeAttrs config ["output" "_module"]);
        stringCfg = lib.mapAttrs (name: value: builtins.toString value) cleanCfg;
        setCfg = lib.mapAttrsToList (name: value: name + "=" + value) stringCfg;
      in
  ...
  zfsprops = {config, ...}: {
    ...
    config = {
      output = let
        cleanCfg = lib.filterAttrs (name: value: value != null) (removeAttrs config ["output" "_module" "prefix"]);
        stringCfg = lib.mapAttrs (name: value: builtins.toString value) cleanCfg;
        setCfg = lib.mapAttrsToList (name: value: name + "=" + value) stringCfg;
      in
        lib.concatMapStrings (x: "-${config.prefix} ${x} \\\n") setCfg;
    };
  };
in {
  options.zpool = {
    ...
    zpoolprops = lib.mkOption {
      type = types.submodule zpoolprops;
      default = {};
    };
    zfsprops = lib.mkOption {
      type = types.submodule zfsprops;
      default = {prefix = "O";};
    };
  ...
  config.zpool = {
    output = let
      cleanZpool = lib.filterAttrs (name: value: value != null) (removeAttrs cfg_zpool ["output"]);
      stringZpool = lib.mapAttrs (name: value: builtins.toString value) cleanZpool;
      setZpool = lib.mapAttrsToList (name: value: name + "=" + value) stringZpool;
    in
      #"zpool create ${cfg_zpool.poolName}" + cfg_zpool.zpoolprops.output;
      lib.concatStringsSep " \\\n    " (
        ["zpool create"]
        ++ (getArgs cfg_zpool.zpoolprops)
        ++ (getArgs cfg_zpool.zfsprops)
        ++ [(builtins.toString cfg_zpool.poolName)]
        ++ (map builtins.toString cfg_zpool.devices)
      )
      + "\n";
  };
}
